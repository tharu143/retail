...
const DEFAULT_PR_LIST_COLUMNS = [
  { key: 'name', label: 'RECEIPT NUMBER' },
...
function PurchaseReceiptList() {
  ...
  const [hiddenDefaults, setHiddenDefaults] = useState(() => {
    try {
      ...
      const configKey = `custom_columns_config_${user}_Purchase Receipt`;
      const saved = localStorage.getItem(configKey) || localStorage.getItem('custom_columns_config_Purchase Receipt');
      if (saved) {
  ...
  const [customColumns, setCustomColumns] = useState(() => {
    try {
      const user = localStorage.getItem('user_id') || localStorage.getItem('user_email') || 'default';
      const configKey = `custom_columns_config_${user}_Purchase Receipt`;
      const savedConfig = localStorage.getItem(configKey) || localStorage.getItem('custom_columns_config_Purchase Receipt');
      if (savedConfig) {
        const parsed = JSON.parse(savedConfig);
        if (Array.isArray(parsed.customColumns)) return parsed.customColumns;
      }
      const saved = localStorage.getItem(`custom_columns_${user}_Purchase Receipt`) || localStorage.getItem('custom_columns_Purchase Receipt');
  ...
  const handleUOMChange = async (arg1, arg2) => {
    ...
    setFormData(prev => {
      ...
      if (isMasterBox) {
        ...
        item.accepted_qty = Math.round((parseFloat(item.custom_box_qty) || 1) * totalPcsInMB);
        ...
        item.custom_box_qty = item.custom_box_qty || 1;
        item.accepted_qty = Math.round((parseFloat(item.custom_box_qty) || 1) * pPerBox);
        ...
        item.accepted_qty = parseFloat(item.custom_box_qty) || 1;
      ...
      item.amount = (item.accepted_qty * currentRate).toFixed(2);
        ...
        if (res.data?.message) {
          ...
          setFormData(currentForm => {
            ...
            if (items[rowIndex] && items[rowIndex].item_code === targetItem.item_code) {
              ...
              cur.amount = (parseFloat(cur.accepted_qty || 1) * (parseFloat(cur.rate) || buyingRate)).toFixed(2);
              items[rowIndex] = cur;
            }
            const total_qty = items.reduce((sum, i) => sum + parseFloat(i.received_qty || 0), 0);
            const net_total = items.reduce((sum, i) => sum + parseFloat(i.amount || 0), 0);
            const totals = recalcTaxesAndTotals(items, net_total, currentForm.taxes, currentForm.additional_discount_percentage, currentForm.discount_amount, currentForm.rounded_total, currentForm.apply_discount_on);
            return { ...currentForm, items, total_qty, net_total, ...totals };
          });
        }
      }
    } catch (e) {
    ...
    setFormData(prev => {
      ...
      if (existingIdx !== -1) {
        ...
        if (isMasterBoxScan) {
          ...
          existingItem.accepted_qty = Math.round(existingItem.custom_box_qty * boxesPerMb * pcsPerBox);
        ...
        const emptyRowIdx = rowIndex !== undefined && rowIndex >= 0 && rowIndex < items.length && !items[rowIndex].item_code ? rowIndex : -1;
        if (emptyRowIdx !== -1) {
  ...
  const paginated = sortedReceipts.slice((currentPage - 1) * pageSize, currentPage * pageSize);
  ...
  }, [isModalOpen, formData, allowedActions, isViewMode, saving, taxesTemplates, isDirty, docName]);
        ...
        <div className="erp-page so-page font-sans bg-[#f8fafc] min-h-screen flex flex-col" style={{ height: '100vh', overflowY: 'auto' }}>
          ...
          <div className="erp-dialog-body so-modal-body" style={{ flex: 1, overflowY: 'auto', padding: '1.25rem 2rem' }}>
            <div className="w-full flex flex-col gap-6">
              ...
              <div className="erp-card so-card" style={{ overflow: 'visible' }}>
                <div className="erp-section-header so-card-header" style={{ padding: '0.8rem 1.25rem' }}>
                  <p className="so-card-title">Product Basket</p>
                  {!isViewMode && (
                    <button onClick={addItemRow} className="erp-button erp-button-secondary so-btn-secondary" style={{ padding: '0.3rem 0.75rem', fontSize: '0.7rem' }}>
                ...
                <div className="so-card-body" style={{ padding: 0 }}>
                  ...
                  )}                        <div className="erp-table-scroll purchase-table-container" style={{ boxShadow: 'none' }}>
  ...
  return (
    <>
      <div className="erp-page so-page">
                  ...
                  <div className="erp-table-scroll purchase-table-container">
                    <table className="erp-table so-table">
                      <thead>
                        <tr>
                          ...
                          <th style={{ width: '48px', textAlign: 'center', verticalAlign: 'middle', padding: '0 4px' }}>
                            <ListCustomizer
                              doctype="Purchase Receipt"
                              defaultColumns={DEFAULT_PR_LIST_COLUMNS}
                              iconOnly
                              onSave={(cols, hidden) => {
                      ...
                      <tbody>
                        {paginated.length === 0 ? (
                          <tr>
                            <td colSpan={DEFAULT_PR_LIST_COLUMNS.length - hiddenDefaults.length + customColumns.length + 1} className="erp-empty so-empty">
                              ...
                              {!hiddenDefaults.includes('supplier_name') && (
                                <td>
                                  <div 
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setFilterSupplier(rec.supplier_name || rec.supplier || '');
                                      setCurrentPage(1);
                                    }}
                              ...
                              {!hiddenDefaults.includes('set_warehouse') && (
                                <td>
                                  <span 
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      if (rec.set_warehouse || rec.custom_branch) {
                                        setFilterName(rec.set_warehouse || rec.custom_branch);
                                        setCurrentPage(1);
                                      }
                                    }}
                                ...
                                <td>
                                  <span 
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      if (rec.posting_date) {
                                        setFilterDateFrom(rec.posting_date);
                                        setFilterDateTo(rec.posting_date);
                                        setCurrentPage(1);
                                      }
                                    }}
                                  ...
                                  >
                                    {rec.posting_date ? format(new Date(rec.posting_date), 'dd-MM-yyyy') : '—'}
                                  </span>
                                </td>
                              )}
                              {!hiddenDefaults.includes('status') && (
        ...
        {isModalOpen && (
          <div className="erp-overlay so-modal-overlay" onClick={closeModal} style={{
            padding: 0,
            zIndex: 10500,
            top: '74px',
            ...
            <div className="so-modal" style={{
              ...
              <div className="erp-dialog-edge so-modal-header" style={{ padding: '0.85rem 2rem', background: '#fff', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
