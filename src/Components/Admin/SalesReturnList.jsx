...
function SalesReturnList() {
  ...
  const fetchReturns = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_BASE}.get_returns_list`, { params: { doctype: 'Sales Invoice' } });
  ...
  const fetchOriginalInvoices = async (q = '') => {
    try {
      ...
      // We look for submitted invoices that are not returns and potentially have not been fully returned
      const res = await axios.get(`/api/resource/Sales Invoice`, {
        params: {
          ...
          fields: '["name", "customer_name", "grand_total", "posting_date", "currency"]',
  ...
  const handleSelectOriginal = async (invName) => {
    try {
      ...
      const res = await axios.get(`${API_BASE}.make_return_doc_from_original`, {
        params: { doctype: 'Sales Invoice', original_name: invName }
      });
      if (res.data.message) {
  ...
  const handleDocumentAction = async (action) => {
    ...
    if (action === 'submit') {
      const result = await Swal.fire({
        ...
        text: 'This will finalize the Sales Return and update inventory/accounts.',
        icon: 'warning',
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc', position: 'relative', fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .status-badge { padding: 4px 12px; borderRadius: 10px; fontSize: 11px; fontWeight: 900; textTransform: uppercase; letterSpacing: 0.05em; }
            ...
            <div style={{ maxWidth: '1400px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h1 style={{ fontSize: '2.25rem', fontWeight: 900, display: 'flex', alignItems: 'center', gap: '1rem', color: '#0f172a' }}>
                  <RotateCcw size={36} style={{ color: themeColor }} /> Sales Returns
                </h1>
                <p style={{ fontSize: '14px', fontWeight: 850, color: '#94a3b8', marginTop: '4px' }}>{returns.length} RETURN TRANSACTIONS DETECTED</p>
              </div>
              <div style={{ display: 'flex', gap: '1rem' }}>
                ...
                <button className="so-btn-primary" style={{ background: themeColor, gap: '0.75rem', height: '56px', padding: '0 2.5rem' }} onClick={startReturnFlow}>
                  <Plus size={22} /> Initiate Sales Return
                </button>
              </div>
            </div>
          </div>
          ...
          <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '3rem 4rem' }}>
            <div style={{ position: 'relative', marginBottom: '2.5rem' }}>
              ...
              <input 
                ...
                placeholder="Probe registry for Return ID, Customer or Original Invoice..." 
                value={searchTerm}
            ...
            <div style={{ background: '#fff', borderRadius: '1.5rem', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
              <table className="so-table">
                <thead style={{ background: '#f8fafc' }}>
                  <tr>
                    <th style={{ padding: '1.5rem 2.5rem' }}>Return Asset</th>
                    <th>Status Node</th>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan="5" style={{ textAlign: 'center', padding: '150px', color: '#cbd5e1', fontWeight: 950, fontSize: '20px' }}>NO RETURN SHARDS DETECTED</td></tr>
                  ) : (
                    paginated.map(r => (
                      <tr key={r.name} onClick={() => loadReturnDetails(r.name)} style={{ cursor: 'pointer' }}>
                        <td style={{ padding: '1.5rem 2.5rem' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                            ...
                            <div>
                              <p style={{ fontWeight: 900, color: '#0f172a' }}>{r.customer_name}</p>
                        ...
                        <td style={{ textAlign: 'right', fontWeight: 950, color: '#ef4444', fontSize: '1rem' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 3000, background: 'rgba(15, 23, 42, 0.8)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
          <div className="animate-in zoom-in-95 duration-300" style={{ background: '#fff', width: '100%', maxWidth: '900px', borderRadius: '2.5rem', overflow: 'hidden', shadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}>
            <div style={{ padding: '3rem', borderBottom: '2px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
               <div>
                  <h2 style={{ fontSize: '1.75rem', fontWeight: 950, color: '#0f172a', tracking: '-0.03em' }}>Select Original Document</h2>
                  <p style={{ fontSize: '13px', fontWeight: 800, color: '#94a3b8', marginTop: '4px' }}>PROBE INVOICE REGISTRY FOR RETURN MAPPING</p>
               </div>
            ...
            <div style={{ padding: '3rem' }}>
               <div style={{ position: 'relative', marginBottom: '2rem' }}>
                  ...
                  <input 
                    ...
                    placeholder="Query Invoice ID or Customer Name..." 
                    value={invoiceSearch}
                    onChange={e => {
                    ...
                    originalInvoices.map(inv => (
                      <div key={inv.name} onClick={() => handleSelectOriginal(inv.name)} className="p-card" style={{ padding: '1.5rem 2rem', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                         <div style={{ display: 'flex', gap: '1.25rem', alignItems: 'center' }}>
                            <div style={{ height: '48px', width: '48px', borderRadius: '14px', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b' }}><Receipt size={22} /></div>
                            <div>
                               <p style={{ fontWeight: 900, color: '#0f172a' }}>{inv.name}</p>
                               <span style={{ fontSize: '12px', fontWeight: 850, color: '#94a3b8' }}>{inv.customer_name} • {inv.posting_date}</span>
                            </div>
                         </div>
                         <div style={{ textAlign: 'right' }}>
                            <p style={{ fontWeight: 950, color: '#0f172a', fontSize: '1.1rem' }}>{inv.currency} {inv.grand_total.toLocaleString()}</p>
                            <span style={{ fontSize: '10px', fontWeight: 950, color: themeColor, textTransform: 'uppercase' }}>MAP RESOURCE <ArrowRight size={10} style={{ marginLeft: '4px' }} /></span>
                         </div>
                      </div>
                    ))
                  )}
        ...
        <div className="animate-in fade-in duration-500" style={{ minHeight: '100vh', background: '#fff' }}>
                 ...
                 <div style={{ display: 'flex', gap: '3rem' }}>
                    ...
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '1rem' }}>
                           <span style={{ background: '#ef4444', color: '#fff', padding: '0.4rem 1.25rem', borderRadius: '10px', fontSize: '11px', fontWeight: 950, textTransform: 'uppercase' }}>Sales Return</span>
                        ...
                        <h1 style={{ fontSize: '3.5rem', fontWeight: 950, letterSpacing: '-0.05em', lineHeight: 1 }}>{selectedReturnDoc.customer_name}</h1>
                        <p style={{ marginTop: '1rem', color: 'rgba(255,255,255,0.5)', fontWeight: 800, fontSize: '13px' }}>Identity Key: {selectedReturnDoc.name || 'NEW_RETURN_DRAFT'}</p>
                    </div>
                 </div>
                 ...
                 {selectedReturnDoc.docstatus === 0 && (
                   <div style={{ display: 'flex', gap: '1rem' }}>
                      <button onClick={() => handleDocumentAction('save')} disabled={saving} className="btn-action" style={{ background: '#1e293b', color: '#fff', border: '2px solid rgba(255,255,255,0.1)' }}>
                         {saving ? <Loader2 size={18} className="animate-spin" /> : <Save size={20} />} Commit Draft
                      </button>
                      <button onClick={() => handleDocumentAction('submit')} disabled={saving} className="btn-action" style={{ background: themeColor, color: '#fff' }}>
                         {saving ? <Loader2 size={18} className="animate-spin" /> : <ShieldCheck size={20} />} Finalize Return
                      </button>
                   </div>
                 )}
                 ...
                 {selectedReturnDoc.docstatus === 1 && (
                   <div style={{ display: 'flex', gap: '1rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(16, 185, 129, 0.15)', padding: '0.75rem 1.5rem', borderRadius: '16px', border: '1.5px solid #10b98140' }}>
                         ...
                         <span style={{ fontWeight: 950, fontSize: '14px', color: '#10b981', tracking: '0.05em' }}>IMMUTABLE REGISTRY ENTRY</span>
                      </div>
                      <button onClick={() => handleDocumentAction('cancel')} disabled={saving} className="btn-action" style={{ background: '#fef2f2', color: '#ef4444', border: '2.5px solid #fee2e2' }}>
                         {saving ? <Loader2 size={18} className="animate-spin" /> : <Trash2 size={20} />} Cancel Transaction
                      </button>
                   </div>
                 )}
              </div>
           ...
           <div style={{ maxWidth: '1400px', margin: '4rem auto', padding: '0 5rem 6rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '4rem' }}>
                 ...
                 {/* Item List Shard */}
                 <div>
                    <h3 style={{ fontSize: '14px', fontWeight: 950, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '2rem' }}>Returned Item Matrix</h3>
                          ...
                          <tbody>
                             {selectedReturnDoc.items?.map((item, idx) => (
                                <tr key={idx}>
                                   ...
                                   <td style={{ textAlign: 'center', fontWeight: 950, color: '#ef4444' }}>
                 ...
                 <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    <div style={{ background: '#0f172a', padding: '3.5rem', borderRadius: '3rem', color: '#fff' }}>
                       <h4 style={{ fontSize: '11px', fontWeight: 950, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', marginBottom: '2.5rem', tracking: '0.2em' }}>Financial Liquidity Impact</h4>
                       <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                             <span style={{ fontSize: '13px', fontWeight: 950, color: 'rgba(255,255,255,0.6)' }}>Total Credit Value</span>
                             <span style={{ fontSize: '2.5rem', fontWeight: 950, letterSpacing: '-0.04em', color: '#ef4444' }}>
                    ...
                    <div style={{ background: '#f8fafc', padding: '3rem', borderRadius: '3rem', border: '3px solid #f1f5f9' }}>
                       <h4 style={{ fontSize: '11px', fontWeight: 950, color: '#94a3b8', textTransform: 'uppercase', marginBottom: '2rem', tracking: '0.15em' }}>Registry Parameters</h4>
                       <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                          <ParamRow icon={Calendar} label="Posting Signal" value={selectedReturnDoc.posting_date} />
                          <ParamRow icon={Hash} label="Original Vector" value={selectedReturnDoc.return_against} />
