...
function SalesReturnList() {
  ...
  const DEFAULT_SRTN_LIST_COLUMNS = [
    { key: 'customer_name', label: 'Customer Node' },
    { key: 'status', label: 'Status' },
    { key: 'return_against', label: 'Original Invoice' },
    { key: 'grand_total', label: 'Credit Value' },
    { key: 'name', label: 'Key' }
  ];
  ...
  const [hiddenDefaults, setHiddenDefaults] = useState(() => {
    try {
      ...
      const configKey = `custom_columns_config_${user}_Sales Return`;
      const saved = localStorage.getItem(configKey) || localStorage.getItem('custom_columns_config_Sales Return');
      if (saved) {
  ...
  const [customColumns, setCustomColumns] = useState(() => {
    try {
      const user = localStorage.getItem('user_id') || localStorage.getItem('user_email') || 'default';
      const configKey = `custom_columns_config_${user}_Sales Return`;
      const savedConfig = localStorage.getItem(configKey) || localStorage.getItem('custom_columns_config_Sales Return');
      if (savedConfig) {
        const parsed = JSON.parse(savedConfig);
        if (Array.isArray(parsed.customColumns)) return parsed.customColumns;
      }
      const saved = localStorage.getItem(`custom_columns_${user}_Sales Return`) || localStorage.getItem('custom_columns_Sales Return');
  ...
  const fetchItemHistory = async (item_code) => {
    try {
      setSelectedModalItems({}); // Reset selections
  ...
  const handleSelectItemFromHistory = (itemRow) => {
    setReturnQueue(prev => {
      ...
      return [...prev, {
        ...
        income_account: itemRow.income_account,
        cost_center: itemRow.cost_center,
        warehouse: itemRow.warehouse,
        uom: itemRow.uom,
        stock_uom: itemRow.stock_uom,
        conversion_factor: itemRow.conversion_factor,
        returnable_qty: itemRow.returnable_qty
      }];
    });
    // Removed setShowItemSearchModal(false) to allow multiple selection
  };
              ...
              <div className="erp-table-scroll so-table-wrapper" style={{ background: '#751010' }}>
                <table className="erp-table so-table" style={{ background: '#751010', color: '#ffffff' }}>
                    ...
                    <tr>
                      ...
                      <th style={{ width: '48px', textAlign: 'center', verticalAlign: 'middle', padding: '0 4px' }}>
                        <ListCustomizer
                          ...
                          defaultColumns={DEFAULT_SRTN_LIST_COLUMNS}
                          iconOnly
                          onSave={(cols, hidden) => {
                  ...
                  <tbody>
                    {loading ? (
                      <tr>
                        <td colSpan={DEFAULT_SRTN_LIST_COLUMNS.length - hiddenDefaults.length + customColumns.length + 1} style={{ textAlign: 'center', padding: '3rem' }}>
                          <Loader2 size={32} className="animate-spin" style={{ color: themeColor, margin: '0 auto' }} />
                        </td>
                      </tr>
                    ) : paginated.length === 0 ? (
                      <tr>
                        <td colSpan={DEFAULT_SRTN_LIST_COLUMNS.length - hiddenDefaults.length + customColumns.length + 1} style={{ textAlign: 'center', padding: '4rem', color: '#94a3b8', fontWeight: 700 }}>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-400">Select invoices/items to return</p>
                </div>
              </div>
                  ...
                  <table className="erp-table w-full text-left border-collapse bg-white rounded-xl overflow-hidden shadow-sm border border-slate-100">
                    <thead className="bg-slate-50">
                      <tr>
                        ...
                        <th className="text-[10px] font-black text-slate-400 uppercase tracking-widest py-3 px-4 border-b border-slate-100">Invoice / Customer</th>
                        <th className="text-[10px] font-black text-slate-400 uppercase tracking-widest py-3 px-4 border-b border-slate-100">Item Details</th>
                        ...
                        return (
                          <tr key={idx} className="hover:bg-slate-50/50 transition-colors border-b border-slate-100 last:border-0 group">
                            ...
                            <td className="py-4 px-4">
                              ...
                              <div className="text-[10px] font-semibold text-sky-500 mt-0.5">{row.customer_name || row.customer || 'Customer'}</div>
                            </td>
                            <td className="py-4 px-4">
                              <div className="text-xs font-black text-slate-800">{row.item_name}</div>
                              <div className="text-[10px] font-semibold text-slate-400 mt-0.5">{row.item_code}</div>
                            </td>
