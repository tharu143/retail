...
function PurchaseOrder() {
    ...
    try {
      ...
      setFormData(prev => {
        ...
        if (existingIdx !== -1) {
          ...
          const piecesPerBox = parseFloat(item.custom_pcs_per_box || item.custom_pieces_per_box || 1) || 1;
          const boxesPerMb = parseFloat(item.custom_boxes_per_master_box || item.boxes_per_master_box || 1) || 1;
          ...
          const boxQty = 1;
          const totalPieces = isMasterBox
            ? Math.round(boxQty * boxesPerMb * piecesPerBox)
            : (isBox ? (boxQty * piecesPerBox) : 1);
          ...
          items[rowIndex] = {
            ...
            uom: selectedUom,
            base_nos_rate: baseRate,
            rate: baseRate,
            last_buying_rate: baseRate,
            qty: totalPieces,
            amount: totalAmount,
            custom_pieces_per_box: piecesPerBox,
            default_pieces_per_box: piecesPerBox,
            custom_boxes_per_master_box: boxesPerMb,
            default_boxes_per_master_box: boxesPerMb,
            custom_box_price: baseRate * piecesPerBox,
            custom_master_box_price: baseRate * piecesPerBox * boxesPerMb,
            custom_box_qty: isBoxOrMb ? boxQty : (piecesPerBox > 0 ? 1 / piecesPerBox : 1),
  ...
  const handleUOMChange = async (uomValue, rowIndex) => {
    ...
    setFormData(prev => {
      ...
      if (matchingBc && matchingBc.barcode) {
        item.temp_barcode = matchingBc.barcode;
      ...
      if (isMasterBox) {
        item.custom_box_qty = item.custom_box_qty || 1;
        item.qty = Math.round((parseFloat(item.custom_box_qty) || 1) * totalPcsInMB);
        ...
        item.custom_box_qty = item.custom_box_qty || 1;
        item.qty = Math.round((parseFloat(item.custom_box_qty) || 1) * pPerBox);
        ...
        item.qty = parseFloat(item.custom_box_qty) || 1;
      ...
      if (targetItem?.item_code) {
        ...
        const res = await axios.get(`${API_PATH}.get_item_buying_rate`, {
          params: {
            ...
            warehouse: formData.set_warehouse || warehouse || undefined,
          ...
          setFormData(currentForm => {
            ...
            if (items[rowIndex] && items[rowIndex].item_code === targetItem.item_code) {
              ...
              if (Array.isArray(newBcList) && newBcList.length > 0) {
                ...
                if (matchingBc && matchingBc.barcode) {
                  cur.temp_barcode = matchingBc.barcode;
            ...
            const totals = calculateTotals(items, currentForm.taxes);
            return { ...currentForm, items, ...totals };
          });
        }
      }
    } catch (e) {
  ...
  const handleSupplierCreate = async (name) => {
    try {
      ...
      const { value: formValues } = await Swal.fire({
        ...
        html: `
          <div style="text-align: left; display: flex; flex-direction: column; gap: 12px; font-size: 12px; margin-top: 8px;">
            <div>
              ...
              <input id="swal_po_supp_name" class="swal2-input !h-10 !m-0 !w-full !text-xs !font-bold !rounded-xl" value="${(name || '').trim()}" placeholder="Enter Supplier Legal / Trade Name" />
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
              <div>
                <label style="font-weight: 800; color: #334155; font-size: 11px; display: block; margin-bottom: 4px;">MOBILE NO</label>
                <input id="swal_po_supp_mobile" class="swal2-input !h-10 !m-0 !w-full !text-xs !rounded-xl" placeholder="e.g. 0501234567" />
              </div>
              <div>
                <label style="font-weight: 800; color: #334155; font-size: 11px; display: block; margin-bottom: 4px;">EMAIL</label>
                <input id="swal_po_supp_email" type="email" class="swal2-input !h-10 !m-0 !w-full !text-xs !rounded-xl" placeholder="supplier@example.com" />
              </div>
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
              <div>
                <label style="font-weight: 800; color: #334155; font-size: 11px; display: block; margin-bottom: 4px;">TAX ID / TRN (15 DIGITS)</label>
                <input id="swal_po_supp_tax" maxlength="15" class="swal2-input !h-10 !m-0 !w-full !text-xs !rounded-xl !font-mono" placeholder="100XXXXXXXXX003" />
              </div>
              <div>
                <label style="font-weight: 800; color: #334155; font-size: 11px; display: block; margin-bottom: 4px;">SUPPLIER GROUP</label>
                <select id="swal_po_supp_group" class="swal2-input !h-10 !m-0 !w-full !text-xs !bg-white !rounded-xl">
                  ${groupOptionsHtml}
                </select>
              </div>
            </div>
        ...
        didOpen: () => {
          const taxInput = document.getElementById('swal_po_supp_tax');
          if (taxInput) {
        ...
        preConfirm: () => {
          const suppName = document.getElementById('swal_po_supp_name')?.value?.trim();
          const mobile = document.getElementById('swal_po_supp_mobile')?.value?.trim();
          const email = document.getElementById('swal_po_supp_email')?.value?.trim();
          const taxId = document.getElementById('swal_po_supp_tax')?.value?.trim();
          const suppGroup = document.getElementById('swal_po_supp_group')?.value;
  ...
  const handleGlobalItemSearch = async (term) => {
    try {
      ...
      console.error("Global item search failed:", e);
      return [];
    }
  };
  ...
  const handleActivateItem = async (item) => {
    try {
      ...
      return res.data?.message?.success || res.data?.success || false;
    } catch (e) {
  ...
  const handleItemSelect = (rawItem, rowIndex) => {
    ...
    setFormData(prev => {
      ...
      if (existingIdx !== -1) {
        ...
        const defaultMasterBoxPrice = parseFloat(item.custom_master_box_price || (rate * pPerBox * boxesPerMb) || 0);
        ...
        const sellMasterBox = parseFloat(item.custom_master_box_selling_price || (sellNos * pPerBox * boxesPerMb) || 0);
        const itemQty = isMasterBox
          ? Math.round(boxesPerMb * pPerBox)
          : (isBox ? pPerBox : 1);
        ...
        const newRow = {
          ...
          default_boxes_per_master_box: boxesPerMb,
          qty: itemQty,
          amount: isMasterBox ? (rate * boxesPerMb * pPerBox) : (isBox ? (rate * pPerBox) : rate),
          custom_box_price: defaultBoxPrice,
          custom_master_box_price: defaultMasterBoxPrice,
          custom_box_qty: 1,
          temp_barcode: item.barcode || item.scanned_barcode || '',
  ...
  const fetchItems = async (query) => {
    try {
      ...
      const wh = formData.set_warehouse || warehouse || localStorage.getItem('warehouse') || '';
  ...
  if (theme === 'legacy') {
    return (
      <div className="classic-root" style={{ position: 'relative', height: '100vh', display: 'flex', flexDirection: 'column', background: '#f8fafc', overflow: 'hidden' }}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-slate-100 p-2">
            ...
            <table className="erp-table classic-table" style={{ width: 'max-content', minWidth: '100%', borderCollapse: 'collapse', background: '#ffffff', tableLayout: 'fixed' }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1.5px solid #cbd5e1' }}>
                    ...
                    return (
                      <th
                        ...
                        style={{
                          ...
                          minWidth: colW,
                          maxWidth: colW,
                          textAlign: col.align || (['rate', 'custom_box_price', 'custom_selling_price', 'custom_selling_price_box', 'discount_amount', 'amount', 'last_purchase_rate'].includes(col.id) ? 'right' : (['uom', 'custom_box_qty', 'custom_pieces_per_box', 'qty'].includes(col.id) ? 'center' : 'left')),
                          padding: '10px 8px',
                          fontSize: '11px',
                          fontWeight: 900,
                          color: '#475569',
                  ...
                  <th style={{ width: '40px', minWidth: '40px', textAlign: 'center', padding: '10px 4px' }}>
                    ...
                    <td className="p-0 relative h-10">
                      <CustomSearchDropdown
                        ...
                        onActivate={async (item) => {
                          const ok = await handleActivateItem(item);
                          if (ok) {
                            const validItems = formData.items.filter(it => it.item_code);
                            handleItemSelect(item, validItems.length);
                          }
                          return ok;
                        }}
                        createOption={(query) => {
                          ...
                          setQuickItemTargetRow(validItems.length);
              ...
              <div className="xl:col-span-5 bg-white rounded-xl border border-slate-200 p-3 shadow-xs flex flex-col justify-between gap-3">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-black text-slate-900">{(formData.total_qty || 0).toFixed(2)}</span>
                  </div>
                </div>
                ...
                <div className="flex items-end justify-between gap-3 pt-1">
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-slate-800 font-bold text-sm">{formatPrice(formData.total)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] font-bold uppercase text-slate-400">TAX</span>
                      <span className="text-slate-600 font-bold text-sm">{formatPrice(formData.tax_total)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    <span className="text-2xl font-black text-emerald-600 leading-none flex items-center gap-0.5 mt-0.5">
                      <DirhamIcon size={18} /> {formatPrice(formData.grand_total)}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white border-t border-slate-100 px-4 py-1 text-[10px] text-slate-400 flex items-center gap-5 flex-shrink-0">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-bold text-emerald-600">{formData.supplier?.supplier_name || formData.supplier || 'Not Selected'}</span>
            </div>
            <div className="flex items-center gap-1.5">
          ...
          <div className="fixed inset-0 z-[10001] bg-slate-900/40 backdrop-blur-[2px] flex justify-end animate-fadeIn">
            <div className="w-[400px] h-full bg-white shadow-2xl flex flex-col animate-slideLeft">
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div>
                  <h3 className="font-black text-slate-800 text-lg uppercase tracking-tight">Active Orders</h3>
                  <p className="text-xs text-slate-400 font-bold mt-0.5">Saved Purchase Order Drafts</p>
                </div>
                <button onClick={() => setShowDraftsList(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400">
                  <X size={18} />
                </button>
              </div>
            </div>
        ...
        {/* IN-PLACE QUICK ITEM CREATE MODAL */}
        <QuickItemCreateModal
          ...
          warehouse={formData.set_warehouse || warehouse}
          onItemCreated={(createdItem) => {
            if (quickItemTargetRow !== null && quickItemTargetRow >= 0) {
              handleItemSelect(createdItem, quickItemTargetRow);
            } else {
              const validItems = formData.items.filter(it => it.item_code);
              handleItemSelect(createdItem, validItems.length);
            }
          }}
        />
      </div>
  ...
  return (
    <>
      <div className="font-sans purchase-container" style={{ height: '100vh', overflowY: 'auto' }}>
                                            ...
                                            {!(isViewOnly || formData.docstatus !== 0) ? (
                                              <div>
                                                <CustomSearchDropdown
                                                  ...
                                                  globalSearch={true}
                                                  onGlobalSearch={handleGlobalItemSearch}
                                                  onActivate={async (activatedItem) => {
                                                    const ok = await handleActivateItem(activatedItem);
                                                    if (ok) {
                                                      handleItemSelect(activatedItem, idx);
                                                    }
                                                    return ok;
                                                  }}
                                                />
