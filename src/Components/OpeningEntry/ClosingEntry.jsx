    ...
    useEffect(() => {
        const fetchInvoicesForClosing = async () => {
            ...
            try {
                ...
                const data = await response.json();
                // Inside fetchInvoicesForClosing, after const data = await response.json();
                console.log('Invoices for Closing Response:', data);

                // Dynamic parsing to handle single/double/triple nesting
                let parsed = data;
                while (parsed && parsed.message && !parsed.status) {
                    parsed = parsed.message;
                }

                if (parsed?.status === 'error') {
                    throw new Error(parsed.message || 'Failed to fetch invoices');
                }

                const result = parsed?.data;
                console.log('Parsed result:', result);  // ADD: Debug log
                console.log('Invoices length:', result?.invoices?.length);  // ADD: Debug log
                                ...
                                <tbody>
                                    {invoicesData.invoices.map((inv) => (
                                        <tr key={inv.name}>
                                            ...
                                            <td>₹{inv.net_total?.toFixed(2) || '0.00'}</td>
                                            <td>₹{inv.total_taxes_and_charges?.toFixed(2) || '0.00'}</td>
                                            <td>₹{inv.grand_total?.toFixed(2) || '0.00'}</td>
                                            <td>
                                                {(inv.items || []).length > 0 ? (
                                                    inv.items.map((item) => (
                                                        <div key={item.item_code}>
                                                            {item.item_name} (Qty: {item.qty}, ₹{item.rate?.toFixed(2) || '0.00'})
                                                        </div>
                                                    ))
                                                ) : (
